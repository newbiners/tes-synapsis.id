...
data: BlogPost[]
  ...
  return (
    <main className="h-128 bg-sky-50 flex items-center justify-between px-3 sm:px-10 gap-10">
      ...
      <section className="w-full flex justify-center h-full items-center">
        {data.map((item, idx) => (
          ...
          >
            <h1 className="text-xl sm:text-4xl text-slate-900 font-semibold sm:w-2/3">
              {item.title}
            </h1>
            <p className="mt-3 text-sm sm:text-2xl">{item.body}</p>
            <ActionButton
              variant="violet"
              className="text-white text-xs sm:text-lg font-semibold w-28 sm:w-48 mt-8"
